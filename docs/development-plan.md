...
- 工作区文件由用户创建，wk 只登记中文名.code-workspace 的预期位置。
- 本项目源码持续提交和推送；wk 创建的用户仓库不自动提交或推送，两者不同。
...
1. 完成 0.1.2 发布认证与公开安装验证。
2. 根据用户 Hyper-V 手动测试反馈修复问题；宿主机验证不能等同于 VM 验收。
3. 旧工作补齐 Trellis、追加组成部分和配置迁移命令。
4. AI 驱动归档：业务验收通过后再移动 work、更新索引和引用，代码不随之移动。
5. operations 关联、来源索引和模板受控升级。
