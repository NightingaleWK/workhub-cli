...
同日完成 npm 发布认证后，0.1.2 发布成功，latest 已更新。公开 registry 隔离安装后，wk --version 返回 0.1.2，install、仅 Trellis 的 init 和 check 均通过。README、开发状态和验证页已同步。上表的待认证描述是首次审计时的事实，保留用于追溯。npm 内已发布的 README 保持打包时版本，后续仓库文档调整不覆盖不可变的发布包。
