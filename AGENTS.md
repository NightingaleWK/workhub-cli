...
- 实现代码变更时运行 `npm run check`、`npm run build`、`npm run check:version` 和 `npm test`。修改打包内容时还需核对 `npm pack` 包含必需模板和 LICENSE。
- 每项已完成工作都通过 Git 提交和推送留痕；包版本号与 Git 标签仅在维护者选定的发布里程碑更新。只有维护者明确要求发布时，才发布 npm 包。
- 版本与标签的来源、检查和发布顺序见 `docs/release-process.md`；普通提交不得因缺少新标签而失败。
...
- 试运行 CLI 时，保持 Trellis、work、code 分离，并明确指定目标目录。测试使用临时根目录和 `WORKHUB_CONFIG_PATH`。
